To: Will Cardwell
CC: Lucy (Lucy@imagophotography.co.uk), Harshit Kaushik
Subject: Re: Blog, GDPR, One-Pager & Reddit

---

Hi Will,

Thanks for the update — good to have clarity on a few things.

**How to Choose a School Photography Company** — confirmed, that was the first blog published and it is now live. We have already shared a set of SEO changes with Senthil for implementation, and those have been communicated to him directly. We will follow up with Senthil to confirm the changes are in place.

**GDPR Blog** — we've reviewed the piece from an SEO perspective. The content itself is solid and we're not suggesting changes to the substance. Our recommendations, in priority order:

- Priority 1: Add an FAQ section (6–8 short Q&As covering consent, opt-outs, and data storage). This is the highest-impact addition for Google and AI search visibility.
- Priority 2: Present the lawful basis section (Legitimate Interest vs Public Task vs Consent) as a simple comparison table rather than paragraph text — easier to read and better for search.
- Priority 3: Add a named author credit and a "Last updated" date, as GDPR guidance continues to evolve.

Are you happy for us to draft the FAQ section? If so, we'll prepare the questions and answers and send them over for your review.

**One-Pager** — thank you for adding the completed blogs. We'll take a look and come back if anything needs clarifying.

**Reddit** — understood, the topics you shared are guides for finding similar discussions, not questions to post directly. That's exactly how we've approached it. We've identified two threads that match:

- r/photography — a school photography discussion where we'd answer as Imago, sharing insight on workflow, lighting, and parent expectations.
  Link: https://www.reddit.com/r/photography/comments/1l9p3tc/school_photography/

- r/UKParenting — UK parents discussing what they buy for school photo packages — exactly the audience we want to reach.
  Link: https://www.reddit.com/r/UKParenting/comments/1lr2bfl/what_do_you_guys_usually_get_when_it_comes_to/

Are these the kind of threads you had in mind? If you're happy to proceed, we'll draft the responses and share them for your sign-off before anything goes live.

**Why Photography Matters** — great, we'll look forward to receiving your draft whenever it's ready.

Best,
Ashish
Damco Digital
